...
namespace DDAPI.ZFB
{
    ...
    public sealed class RSAFromPkcs8
    {
        ...
        public static string RSASignCharSet(string data, string privateKey, string charset)
        {
            try
            {
                // Core.LogResult("RSASignCharSet开始" + DateTime.Now.ToString());
                RSACryptoServiceProvider.UseMachineKeyStore = true;
                RSACryptoServiceProvider rsaCsp = LoadCertificateString(privateKey);
                //  Core.LogResult("RSASignCharSet开始2" + DateTime.Now.ToString());
                byte[] dataBytes = null;
                ...
                //  Core.LogResult("RSASignCharSet开始3" + DateTime.Now.ToString());
                byte[] signatureBytes = rsaCsp.SignData(dataBytes, "SHA1");
                //  Core.LogResult("RSASignCharSet开始4" + DateTime.Now.ToString());
                return Convert.ToBase64String(signatureBytes);
            }
            catch (Exception ex)
            {
                // Core.LogResult(ex.ToString() + DateTime.Now.ToString());
                return "";
            }
        }
